	...
	describe('getPiecesSpanningSegmentBeforePart', () => {
		it('has two Parts with each two Pieces, has to find "after" both Parts, returns spanning pieces of both Parts', () => {
			...
			const partOne: Part = EntityDefaultFactory.createPart({
				id: '1',
				rank: 1,
				pieces: [EntityDefaultFactory.createPiece({ id: '1' } as PieceInterface), pieceTwo],
			} as PartInterface)

			const pieceFour: Piece = EntityDefaultFactory.createPiece({
				id: '4',
				pieceLifespan: PieceLifespan.SPANNING_UNTIL_SEGMENT_END,
			...
			const partTwo: Part = EntityDefaultFactory.createPart({
				id: '2',
				rank: 2,
				pieces: [EntityDefaultFactory.createPiece({ id: '3' } as PieceInterface), pieceFour],
			} as PartInterface)

			const partToSearchBefore: Part = EntityDefaultFactory.createPart({ id: '3', rank: 3 } as PartInterface)

			const testee: Segment = EntityDefaultFactory.createSegment({
		...
		it('has two Parts with each two Pieces, has to find before second Part, returns only spanning piece of first Part', () => {
			...
			const partOne: Part = EntityDefaultFactory.createPart({
				id: '1',
				rank: 1,
				pieces: [EntityDefaultFactory.createPiece({ id: '1' } as PieceInterface), piece],
			} as PartInterface)
			const partTwo: Part = EntityDefaultFactory.createPart({
				id: '2',
				rank: 2,
