...
export class RundownLockService implements RundownService {
	...
	private assertNotLocked(action: string): void {
		...
		if (currentTime <= this.lastActionExecutedAt + MINIMUM_TIME_BETWEEN_ACTIONS_IN_MS) {
			throw new RundownIsLockedException(`Unable to do action: "${action}" - it has yet to be ${MINIMUM_TIME_BETWEEN_ACTIONS_IN_MS} ms since last action`)
		}
	...
	public executeAdLibPiece(rundownId: string, adLibPieceId: string): Promise<void> {
	 ...
	 */
	public setNext(rundownId: string, segmentId: string, partId: string): Promise<void> {
