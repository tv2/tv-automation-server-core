		...
		describe('for active Part', () => {
			describe('it creates a group for active Part', () => {
				it('sets correct active group id for the active Part', () => {
					...
					const result: TimelineObjectGroup | undefined = timeline.timelineGroups.find(group => group.id === expectedGroupIdForPart)
				...
				it('sets TimelineEnable.start set to now', () => {
					...
					const result: TimelineObjectGroup = timeline.timelineGroups.find(group => group.id.includes(ACTIVE_GROUP_PREFIX))!
			...
			describe('active Part has one Piece', () => {
				describe('creates a Piece control group on the active group', () => {
					it('sets correct control group id for Piece on active group', () => {
						const piece: Piece = EntityDefaultFactory.createPiece({ transitionType: TransitionType.NO_TRANSITION } as PieceInterface)
						...
						const activeGroup: TimelineObjectGroup = timeline.timelineGroups.find(group => group.id.includes(ACTIVE_GROUP_PREFIX))!
						const expectedControlIdForPiece = `${activeGroup.id}${PIECE_CONTROL_INFIX}${piece.id}`
						const controlGroup: TimelineObject | undefined = activeGroup.children.find(child => child.id === expectedControlIdForPiece)
					...
					it('sets correct parentGroup id', () => {
						...
						const activeGroup: TimelineObjectGroup = timeline.timelineGroups.find(group => group.id.includes(ACTIVE_GROUP_PREFIX))!
						const controlObject: TimelineObject = activeGroup.children.find(child => child.id.includes(PIECE_CONTROL_INFIX))!
					...
					it('sets layer to Piece.layer', () => {
						...
						const piece: Piece = EntityDefaultFactory.createPiece({ layer, transitionType: TransitionType.NO_TRANSITION } as PieceInterface)
						const segment: Segment = createSegmentWithPieces([piece])
						const rundown: Rundown = EntityDefaultFactory.createActiveRundown([segment])
					...
					describe('creates TimelineEnable for IN_TRANSITION Piece', () => {
						describe('active Part has an "inTransitionStart"', () => {
							it('sets TimelineEnable.start to Part.timings.inTransitionStart + Piece.start', () => {
								const piece: Piece = EntityDefaultFactory.createPiece({ start: 10, transitionType: TransitionType.IN_TRANSITION } as PieceInterface)
							...
							it('sets TimelineEnable.duration to Piece.duration', () => {
								const piece: Piece = EntityDefaultFactory.createPiece({ duration: 15, transitionType: TransitionType.IN_TRANSITION } as PieceInterface)
                                const part: Part = EntityDefaultFactory.createPart({ pieces: [piece] } as PartInterface)
						...
						describe('active Part does not have an "inTransitionStart"', () => {
							it('does not create any groups for Piece', () => {
								const piece: Piece = EntityDefaultFactory.createPiece({ transitionType: TransitionType.IN_TRANSITION } as PieceInterface)
								const segment: Segment = createSegmentWithPieces([piece])
								const rundown: Rundown = EntityDefaultFactory.createActiveRundown([segment])
								...
								const activeGroup: TimelineObjectGroup = timeline.timelineGroups.find(group => group.id.includes(ACTIVE_GROUP_PREFIX))!
								const expectedControlIdForPiece = `${activeGroup.id}${PIECE_CONTROL_INFIX}${piece.id}`
								const controlObject: TimelineObject | undefined = activeGroup.children.find(child => child.id === expectedControlIdForPiece)
					...
					describe('creates TimelineEnable for OUT_TRANSITION Piece', () => {
						describe('active Part has a "KeepAliveDuration"', () => {
							describe('active Part has a PostRollDuration', () => {
								it('sets TimelineEnable.start to activeGroup.end - Part.keepAliveDuration - Part.postRollDuration', () => {
									const piece: Piece = EntityDefaultFactory.createPiece({ transitionType: TransitionType.OUT_TRANSITION } as PieceInterface)

									const postRollDuration: number = 20
									const keepAliveDuration: number = 30

                                    const part: Part = EntityDefaultFactory.createPart({ outTransition: { keepAliveDuration }, pieces: [piece] } as PartInterface)
									...
									expect(controlObject.enable.start).toBe(`#${activeGroup.id}.end - ${keepAliveDuration} - ${postRollDuration}`)
								})
							})
							...
							describe('active Part does not have a PostRollDuration', () => {
								it('sets TimelineEnable.start to activeGroup.end - Part.keepAliveDuration', () => {
									...
									expect(controlObject.enable.start).toBe(`#${activeGroup.id}.end - ${keepAliveDuration}`)
								})
							})
						})
						...
						describe('active Part does not have a "KeepAliveDuration"', () => {
							it('does not create any groups for Piece', () => {
								...
								const activeGroup: TimelineObjectGroup = timeline.timelineGroups.find(group => group.id.includes(ACTIVE_GROUP_PREFIX))!
								const controlObject: TimelineObject | undefined = activeGroup.children.find(child => child.id === `${activeGroup.id}${PIECE_CONTROL_INFIX}${piece.id}`)
					...
					describe('creates TimelineEnable for NO_TRANSITION Piece', () => {
						it('sets TimelineEnable.start to Piece.start', () => {
							const piece: Piece = EntityDefaultFactory.createPiece({ start: 5, transitionType: TransitionType.NO_TRANSITION } as PieceInterface)
							const segment: Segment = createSegmentWithPieces([piece])
							const rundown: Rundown = EntityDefaultFactory.createActiveRundown([segment])
						...
						describe('Piece has a duration', () => {
							it('sets TimelineEnable.duration to Piece.duration', () => {
								const piece: Piece = EntityDefaultFactory.createPiece({ duration: 50, transitionType: TransitionType.NO_TRANSITION } as PieceInterface)
								const segment: Segment = createSegmentWithPieces([piece])
								const rundown: Rundown = EntityDefaultFactory.createActiveRundown([segment])
						...
						describe('Piece does not have a duration', () => {
							describe('active Part has PostRoll', () => {
								it('sets TimelineEnable.duration activeGroup.end - Part.timings.postRollDuration', () => {
									...
									expect(controlObject.enable.duration).toBe(`#${activeGroup.id} - ${postRollDuration}`)
								})
							})
					...
					describe('controlGroup has TimelineEnable.start === zero && Piece has PreRoll', () => {
						describe('creates PreRollControlGroup for Piece', () => {
							it('sets id to correct id for PreRollControlGroup', () => {
								const piece: Piece = EntityDefaultFactory.createPiece({ start: 0, preRollDuration: 10, transitionType: TransitionType.NO_TRANSITION } as PieceInterface)
								const segment: Segment = createSegmentWithPieces([piece])
								const rundown: Rundown = EntityDefaultFactory.createActiveRundown([segment])
								...
								const activeGroup: TimelineObjectGroup = timeline.timelineGroups.find(group => group.id.includes(ACTIVE_GROUP_PREFIX))!
								const controlObject: TimelineObject = activeGroup.children.find(child => child.id === `${activeGroup.id}${PIECE_CONTROL_INFIX}${piece.id}`)!
								const expectedPreRollIdForPiece: string = `${PIECE_PRE_ROLL_PREFIX}${controlObject.id}`
								const preRollObject: TimelineObject | undefined = activeGroup.children.find(child => child.id === expectedPreRollIdForPiece)
							...
							it('sets TimelineEnable.start to "activeGroup.id.start"', () => {
								...
								const activeGroup: TimelineObjectGroup = timeline.timelineGroups.find(group => group.id.includes(ACTIVE_GROUP_PREFIX))!
								const preRollObject: TimelineObject = activeGroup.children.find(child => child.id.includes(PIECE_PRE_ROLL_PREFIX))!
							...
							it('updates controlPiece to start at PreRollControlGroup + Piece.preRollDuration', () => {
								...
								expect(controlObject.enable.start).toBe(`#${preRollObject.id} + ${piece.preRollDuration}`)
							})
						})
					})
				})
				...
				describe('create a Piece child group on the active group', () => {
					it('sets correct Piece group id for Piece on active group', () => {
						...
						const childGroup: TimelineObject | undefined = activeGroup.children.find(child => child.id === expectedChildGroupIdForPiece)
					...
					it('sets correct parentGroup id', () => {
						...
						const activeGroup: TimelineObjectGroup = timeline.timelineGroups.find(group => group.id.includes(ACTIVE_GROUP_PREFIX))!
						const childGroup: TimelineObject = activeGroup.children.find(child => child.id.includes(PIECE_GROUP_INFIX))!
					...
					describe('Piece has PreRoll', () => {
						it('sets TimelineEnable.start PieceControlGroup.start - Piece.preRollDuration', () => {
							const piece: Piece = EntityDefaultFactory.createPiece({ preRollDuration: 20, transitionType: TransitionType.NO_TRANSITION } as PieceInterface)
							const segment: Segment = createSegmentWithPieces([piece])
							const rundown: Rundown = EntityDefaultFactory.createActiveRundown([segment])
							...
							expect(childGroup.enable.start).toBe(`#${controlObject.id}.start - ${piece.preRollDuration}`)
						})
					})
					...
					describe('Piece has PostRoll', () => {
						it('sets TimelineEnable.end to PieceControlGroup.end - Piece.postRollDuration', () => {
							const piece: Piece = EntityDefaultFactory.createPiece({ postRollDuration: 30, transitionType: TransitionType.NO_TRANSITION } as PieceInterface)
							const segment: Segment = createSegmentWithPieces([piece])
							const rundown: Rundown = EntityDefaultFactory.createActiveRundown([segment])
					...
					describe('Piece has a TimelineObject', () => {
						it('sets the id of the TimelineObject to be pieceChildGroup.id_piece.id_timelineObject.id', () => {
							...
							const piece: Piece = EntityDefaultFactory.createPiece({ timelineObjects: [timelineObject], transitionType: TransitionType.NO_TRANSITION } as PieceInterface)
							const segment: Segment = createSegmentWithPieces([piece])
							const rundown: Rundown = EntityDefaultFactory.createActiveRundown([segment])
							...
							const activeGroup: TimelineObjectGroup = timeline.timelineGroups.find(group => group.id.includes(ACTIVE_GROUP_PREFIX))!
							const childGroup: TimelineObjectGroup = activeGroup.children.find(child => child.id.includes(PIECE_GROUP_INFIX))! as TimelineObjectGroup
					...
					describe('Piece has five TimelineObjects', () => {
						it('adds all five TimelineObjects to the children of the Piece child group', () => {
                            ...
                            const piece: Piece = EntityDefaultFactory.createPiece({ timelineObjects, transitionType: TransitionType.NO_TRANSITION } as PieceInterface)
                            const segment: Segment = createSegmentWithPieces([piece])
			...
			describe('active Part has five Pieces', () => {
				it('creates five Piece control groups on the active group', () => {
                    const pieces: Piece[] = [
                        EntityDefaultFactory.createPiece({ id: '1', transitionType: TransitionType.NO_TRANSITION } as PieceInterface),
                        EntityDefaultFactory.createPiece({ id: '2', transitionType: TransitionType.NO_TRANSITION } as PieceInterface),
                        EntityDefaultFactory.createPiece({ id: '3', transitionType: TransitionType.NO_TRANSITION } as PieceInterface),
                        EntityDefaultFactory.createPiece({ id: '4', transitionType: TransitionType.NO_TRANSITION } as PieceInterface),
                        EntityDefaultFactory.createPiece({ id: '5', transitionType: TransitionType.NO_TRANSITION } as PieceInterface),
					]
                    ...
                    const activeGroup: TimelineObjectGroup = timeline.timelineGroups.find(group => group.id.includes(ACTIVE_GROUP_PREFIX))!
                    const controlGroups: TimelineObject[] = activeGroup.children.filter(child => child.id.includes(PIECE_CONTROL_INFIX))
				...
				it('creates five Piece child groups on the active group', () => {
                    ...
                    const activeGroup: TimelineObjectGroup = timeline.timelineGroups.find(group => group.id.includes(ACTIVE_GROUP_PREFIX))!
                    const childGroups: TimelineObject[] = activeGroup.children.filter(child => child.id.includes(PIECE_GROUP_INFIX))
				...
				describe('four of the Pieces are infinite Pieces', () => {
					it('still creates groups for five Pieces', () => {
                        const pieces: Piece[] = [
                            EntityDefaultFactory.createPiece({ id: '1', pieceLifespan: PieceLifespan.WITHIN_PART, transitionType: TransitionType.NO_TRANSITION } as PieceInterface),
                            EntityDefaultFactory.createPiece({ id: '2', pieceLifespan: PieceLifespan.STICKY_UNTIL_RUNDOWN_CHANGE, transitionType: TransitionType.NO_TRANSITION } as PieceInterface),
                            EntityDefaultFactory.createPiece({ id: '3', pieceLifespan: PieceLifespan.SPANNING_UNTIL_RUNDOWN_END, transitionType: TransitionType.NO_TRANSITION } as PieceInterface),
                            EntityDefaultFactory.createPiece({ id: '4', pieceLifespan: PieceLifespan.STICKY_UNTIL_SEGMENT_CHANGE, transitionType: TransitionType.NO_TRANSITION } as PieceInterface),
                            EntityDefaultFactory.createPiece({ id: '5', pieceLifespan: PieceLifespan.SPANNING_UNTIL_SEGMENT_END, transitionType: TransitionType.NO_TRANSITION } as PieceInterface),
                        ]
                        const segment: Segment = createSegmentWithPieces(pieces)
                        const rundown: Rundown = EntityDefaultFactory.createActiveRundown([segment])
		...
		describe('Rundown has a previous Part', () => {
			describe('previous Part does not have an executedAt', () => {
				it('throws an error', () => {
                    ...
                    const segment: Segment = EntityDefaultFactory.createSegment({ parts: [previousPart, activePart]} as SegmentInterface)
                    const rundown: Rundown = EntityDefaultFactory.createActiveRundown([segment])
                    rundown.takeNext()
			...
			describe('it creates a group for previous Part', () => {
                ...
                it('sets priority of the group to low', () => {
                    ...
                    const result: TimelineObjectGroup = timeline.timelineGroups.find(group => group.id.includes(PREVIOUS_GROUP_PREFIX))!
                ...
                describe('active Part has a "previousPartContinueIntoPartDuration"', () => {
                    it('sets the TimelineEnable.end to activeGroup.start + active Part.previousPartContinueIntoPartDuration', () => {
						...
						when(activePartSpy.getTimings()).thenReturn({ previousPartContinueIntoPartDuration } as PartTimings)
                        ...
                        expect(result.enable.end).toBe(`#${activeGroupId}.start + ${previousPartContinueIntoPartDuration}`)
                    })
                })
                ...
                describe('active Part does not have a "previousPartContinueIntoPartDuration"', () => {
                    it('sets the TimelineEnable.end to activeGroup.start + 0', () => {
                        ...
                        const activePartSpy: Part = spy(activePart)
                        when(activePartSpy.getTimings()).thenReturn({ previousPartContinueIntoPartDuration: 0 } as PartTimings)
                ...
                describe('previous Part has a Piece', () => {
                    describe('creates a Piece control group on the previous group', () => {
                        it('sets correct control group id for Piece on previous group', () => {
                            const piece: Piece = EntityDefaultFactory.createPiece({ transitionType: TransitionType.NO_TRANSITION } as PieceInterface)
                            const previousPart: Part = EntityDefaultFactory.createPart({ id: 'previousId', pieces: [piece] } as PartInterface)
                            const activePart: Part = EntityDefaultFactory.createPart({ id: 'activeId' } as PartInterface)
                            const segment: Segment = EntityDefaultFactory.createSegment({ parts: [previousPart, activePart]} as SegmentInterface)
                            ...
                            const previousGroup: TimelineObjectGroup = timeline.timelineGroups.find(group => group.id.includes(PREVIOUS_GROUP_PREFIX))!
                            const expectedControlIdForPiece = `${previousGroup.id}${PIECE_CONTROL_INFIX}${piece.id}`
                            const controlGroup: TimelineObject | undefined = previousGroup.children.find(child => child.id === expectedControlIdForPiece)
                        ...
                        it('sets correct parentGroup id', () => {
                            ...
                            const previousGroup: TimelineObjectGroup = timeline.timelineGroups.find(group => group.id.includes(PREVIOUS_GROUP_PREFIX))!
                            const controlGroup: TimelineObject = previousGroup.children.find(child => child.id.includes(PIECE_CONTROL_INFIX))!
                        ...
                        describe('creates TimelineEnable for IN_TRANSITION Piece', () => {
                            ...
                            describe('previous Part does not have an "inTransitionStart"', () => {
                                it('does not create any groups for Piece', () => {
                                    ...
                                    const previousGroup: TimelineObjectGroup = timeline.timelineGroups.find(group => group.id.includes(PREVIOUS_GROUP_PREFIX))!
                                    const controlGroup: TimelineObject | undefined = previousGroup.children.find(child => child.id.includes(PIECE_CONTROL_INFIX))
                        ...
                        describe('creates TimelineEnable for OUT_TRANSITION Piece', () => {
                            describe('previous Part has a "KeepAliveDuration"', () => {
                                describe('previous Part has a PostRollDuration', () => {
                                    it('sets TimelineEnable.start to previousGroup.end - Part.keepAliveDuration - previous Part.postRoll', () => {
                                        ...
                                        const piece: Piece = EntityDefaultFactory.createPiece({ transitionType: TransitionType.OUT_TRANSITION } as PieceInterface)
                                        const previousPart: Part = EntityDefaultFactory.createPart({ id: 'previousId', outTransition: { keepAliveDuration }, pieces: [piece] } as PartInterface)

                                        const previousPartSpy: Part = spy(previousPart)
                                        when(previousPartSpy.getTimings()).thenReturn({ postRollDuration } as PartTimings)
                                        ...
                                        const previousGroup: TimelineObjectGroup = timeline.timelineGroups.find(group => group.id.includes(PREVIOUS_GROUP_PREFIX))!
                                        const controlObject: TimelineObject = previousGroup.children.find(child => child.id.includes(PIECE_CONTROL_INFIX))!

                                        expect(controlObject.enable.start).toBe(`#${previousGroup.id}.end - ${keepAliveDuration} - ${postRollDuration}`)
                                    })
                                })
                                ...
                                describe('previous Part does not have a PostRollDuration', () => {
                                    it('sets TimelineEnable.start to previousGroup.end - previous Part.keepAliveDuration', () => {
                                        ...
                                        expect(controlObject.enable.start).toBe(`#${previousGroup.id}.end - ${keepAliveDuration}`)
                                    })
                                })
                            })
                            ...
                            describe('previous Part does not have a "KeepAliveDuration"', () => {
                                it('does not create any groups for Piece', () => {
                                    ...
                                    const previousGroup: TimelineObjectGroup = timeline.timelineGroups.find(group => group.id.includes(PREVIOUS_GROUP_PREFIX))!
                                    const controlObject: TimelineObject | undefined = previousGroup.children.find(child => child.id.includes(PIECE_CONTROL_INFIX))
                        ...
                        describe('creates TimelineEnable for NO_TRANSITION Piece', () => {
                            it('sets TimelineEnable.start to Piece.start', () => {
                                const piece: Piece = EntityDefaultFactory.createPiece({ start: 10, transitionType: TransitionType.NO_TRANSITION } as PieceInterface)
                                const previousPart: Part = EntityDefaultFactory.createPart({ id: 'previousId', pieces: [piece] } as PartInterface)
                            ...
                            describe('Piece has a duration', () => {
                                it('sets TimelineEnable.duration to Piece.duration', () => {
                                    const piece: Piece = EntityDefaultFactory.createPiece({ duration: 15, transitionType: TransitionType.NO_TRANSITION } as PieceInterface)
                                    const previousPart: Part = EntityDefaultFactory.createPart({ id: 'previousId', pieces: [piece] } as PartInterface)
                            ...
                            describe('Piece does not have a duration', () => {
                                describe('previous Part has PostRoll', () => {
                                    it('sets TimelineEnable.duration to previousGroup - Part.timings.postRollDuration', () => {
                                        ...
                                        expect(controlObject.enable.duration).toBe(`#${previousGroup.id} - ${postRollDuration}`)
                                    })
                                })
                        ...
                        describe('controlGroup has TimelineEnable.start === zero && Piece has PreRoll', () => {
                            describe('creates PreRollControlGroup for Piece', () => {
                                it('sets id to correct id for PreRollControlGroup', () => {
                                    ...
                                    const controlObject: TimelineObject = previousGroup.children.find(child => child.id.includes(PIECE_CONTROL_INFIX))!
                                    const expectedPreRollIdForPiece: string = `${PIECE_PRE_ROLL_PREFIX}${controlObject.id}`
                                    const preRollObject: TimelineObject | undefined = previousGroup.children.find(child => child.id === expectedPreRollIdForPiece)
                                ...
                                it('sets TimelineEnable.start to "previousGroup.id.start"', () => {
                                    ...
                                    const previousGroup: TimelineObjectGroup = timeline.timelineGroups.find(group => group.id.includes(PREVIOUS_GROUP_PREFIX))!
                                    const preRollObject: TimelineObject = previousGroup.children.find(child => child.id.includes(PIECE_PRE_ROLL_PREFIX))!
                    ...
                    describe('create a Piece child group on the previous group', () => {
                        it('sets correct Piece group id for Piece on previous group', () => {
                            ...
                            const childGroup: TimelineObject | undefined = previousGroup.children.find(child => child.id === expectedChildGroupIdForPiece)
                        ...
                        it('sets correct parentGroup id', () => {
                            ...
                            const previousGroup: TimelineObjectGroup = timeline.timelineGroups.find(group => group.id.includes(PREVIOUS_GROUP_PREFIX))!
                            const childGroup: TimelineObject = previousGroup.children.find(child => child.id.includes(PIECE_GROUP_INFIX))!
                        ...
                        describe('Piece has PreRoll', () => {
                            it('sets TimelineEnable.start PieceControlGroup.start - Piece.preRollDuration', () => {
                                const piece: Piece = EntityDefaultFactory.createPiece({ preRollDuration: 10, transitionType: TransitionType.NO_TRANSITION } as PieceInterface)
                                const previousPart: Part = EntityDefaultFactory.createPart({ id: 'previousId', pieces: [piece] } as PartInterface)
                                ...
                                expect(childGroup.enable.start).toBe(`#${controlGroup.id}.start - ${piece.preRollDuration}`)
                            })
                        })
                        ...
                        describe('Piece has PostRoll', () => {
                            it('sets TimelineEnable.end to PieceControlGroup.end - Piece.postRollDuration', () => {
                                ...
                                expect(childGroup.enable.end).toBe(`#${controlGroup.id}.end - ${piece.postRollDuration}`)
                            })
                        })
                        ...
                        describe('Piece has a TimelineObject', () => {
                            it('sets the id of the TimelineObject to be pieceChildGroup.id_piece.id_timelineObject.id', () => {
                                ...
                                const previousGroup: TimelineObjectGroup = timeline.timelineGroups.find(group => group.id.includes(PREVIOUS_GROUP_PREFIX))!
                                const childGroup: TimelineObjectGroup = previousGroup.children.find(child => child.id.includes(PIECE_GROUP_INFIX))! as TimelineObjectGroup
                                const result: TimelineObject = childGroup.children[0]
			...
			describe('previous Part has Pieces', () => {
				describe('previous Part has five Pieces, but one of them is an infinite Piece', () => {
					it('only creates groups for four Pieces', () => {
                        const pieces: Piece[] = [
                            EntityDefaultFactory.createPiece({ pieceLifespan: PieceLifespan.STICKY_UNTIL_RUNDOWN_CHANGE, transitionType: TransitionType.NO_TRANSITION } as PieceInterface),
                            EntityDefaultFactory.createPiece({ pieceLifespan: PieceLifespan.WITHIN_PART, transitionType: TransitionType.NO_TRANSITION } as PieceInterface),
                            EntityDefaultFactory.createPiece({ pieceLifespan: PieceLifespan.WITHIN_PART, transitionType: TransitionType.NO_TRANSITION } as PieceInterface),
                            EntityDefaultFactory.createPiece({ pieceLifespan: PieceLifespan.WITHIN_PART, transitionType: TransitionType.NO_TRANSITION } as PieceInterface),
                            EntityDefaultFactory.createPiece({ pieceLifespan: PieceLifespan.WITHIN_PART, transitionType: TransitionType.NO_TRANSITION } as PieceInterface)
						]
                        const previousPart: Part = EntityDefaultFactory.createPart({ id: 'previousId', pieces } as PartInterface)
                        ...
                        const previousGroup: TimelineObjectGroup = timeline.timelineGroups.find(group => group.id.includes(PREVIOUS_GROUP_PREFIX))!
                        const controlGroups: TimelineObject[] = previousGroup.children.filter(child => child.id.includes(PIECE_CONTROL_INFIX))
				...
				describe('previous Part has five Pieces, but all of them are infinite Pieces', () => {
					it('does not create any groups for the Pieces', () => {
                        const pieces: Piece[] = [
                            EntityDefaultFactory.createPiece({ pieceLifespan: PieceLifespan.STICKY_UNTIL_RUNDOWN_CHANGE, transitionType: TransitionType.NO_TRANSITION } as PieceInterface),
                            EntityDefaultFactory.createPiece({ pieceLifespan: PieceLifespan.STICKY_UNTIL_SEGMENT_CHANGE, transitionType: TransitionType.NO_TRANSITION } as PieceInterface),
                            EntityDefaultFactory.createPiece({ pieceLifespan: PieceLifespan.SPANNING_UNTIL_RUNDOWN_END, transitionType: TransitionType.NO_TRANSITION } as PieceInterface),
                            EntityDefaultFactory.createPiece({ pieceLifespan: PieceLifespan.SPANNING_UNTIL_SEGMENT_END, transitionType: TransitionType.NO_TRANSITION } as PieceInterface),
                            EntityDefaultFactory.createPiece({ pieceLifespan: PieceLifespan.STICKY_UNTIL_RUNDOWN_CHANGE, transitionType: TransitionType.NO_TRANSITION } as PieceInterface)
                        ]
                        const previousPart: Part = EntityDefaultFactory.createPart({ id: 'previousId', pieces } as PartInterface)
		...
		describe('active Part has autoNext', () => {
			describe('active Part has an expected duration longer than zero', () => {
				describe('active Part has "delayStartOfPiecesDuration', () => {
					it('sets TimelineEnable.duration of the active group to active Part.expectedDuration + active Part.timings.delayStartOfPiecesDuration', () => {
                        ...
                        const activePart: Part = EntityDefaultFactory.createPart({ expectedDuration: 15, autoNext: true } as PartInterface)
						const activePartSpy: Part = spy(activePart)
                        ...
                        expect(activeGroup.enable.duration).toBe(activePart.expectedDuration + delayStartOfPiecesDuration)
					})
				})
				...
				it('sets Timeline.autoNext.timeUntilAutoNextInMs to be now + active Part.expected duration + active Part.delayStartOffPiecesDuration - next Part.previousPartContinueIntoPartDuration', () => {
					...
					when(nextPartSpy.getTimings()).thenReturn({ previousPartContinueIntoPartDuration: continueIntoPartDuration } as PartTimings)

					const segment: Segment = EntityDefaultFactory.createSegment({ parts: [activePart, nextPart]} as SegmentInterface)
					const rundown: Rundown = EntityDefaultFactory.createActiveRundown([segment])
					...
					const expectedPointInTimeToTakeNext: number = now + activePart.expectedDuration + delayStartOfPiecesDuration - continueIntoPartDuration
				...
				describe('it creates a group for next Part', () => {
					it('sets correct next group id for the next Part', () => {
                        const activePart: Part = EntityDefaultFactory.createPart({ id: 'activePart', expectedDuration: 15, autoNext: true } as PartInterface)
                        ...
                        const nextGroup: TimelineObjectGroup | undefined = timeline.timelineGroups.find(group => group.id === expectNextGroupId)
					...
					it('sets priority of the group to high', () => {
                        ...
                        const nextGroup: TimelineObjectGroup = timeline.timelineGroups.find(group => group.id.includes(NEXT_GROUP_PREFIX))!
					...
					describe('next Part has "previousPartContinueIntoPartDuration', () => {
						it('sets the TimelineEnable.start to be activeGroup.end - next Part.previousPartContinueIntoPartDuration', () => {
							...
							const nextPartSpy: Part = spy(nextPart)
							when(nextPartSpy.getTimings()).thenReturn({ previousPartContinueIntoPartDuration } as PartTimings)
                            ...
                            expect(nextGroup.enable.start).toBe(`#${activeGroup.id}.end - ${previousPartContinueIntoPartDuration}`)
						})
					})
					...
					describe('next Part has a Piece', () => {
						describe('creates a Piece control group on the next group', () => {
							it('sets correct control group id for Piece on next group', () => {
                                ...
                                const piece: Piece = EntityDefaultFactory.createPiece({ transitionType: TransitionType.NO_TRANSITION } as PieceInterface)
                                const nextPart: Part = EntityDefaultFactory.createPart({ id: 'nextPart', pieces: [piece] } as PartInterface)
                                ...
                                const controlGroup: TimelineObject | undefined = nextGroup.children.find(child => child.id === expectedControlIdForPiece)
							...
							it('sets correct parentGroup id', () => {
                                ...
                                const nextGroup: TimelineObjectGroup = timeline.timelineGroups.find(group => group.id.includes(NEXT_GROUP_PREFIX))!
                                const controlGroup: TimelineObject = nextGroup.children.find(child => child.id.includes(PIECE_CONTROL_INFIX))!
							...
							describe('creates TimelineEnable for IN_TRANSITION Piece', () => {
								describe('next Part has an "inTransitionStart"', () => {
									...
									it('sets TimelineEnable.duration to Piece.duration', () => {
                                        ...
                                        const piece: Piece = EntityDefaultFactory.createPiece({ duration: 30, transitionType: TransitionType.IN_TRANSITION } as PieceInterface)
                                        const nextPart: Part = EntityDefaultFactory.createPart({ id: 'nextPart', pieces: [piece] } as PartInterface)
								...
								describe('next Part does not have an "inTransitionStart"', () => {
									it('does not create any groups for Piece', () => {
                                        ...
                                        const nextGroup: TimelineObjectGroup = timeline.timelineGroups.find(group => group.id.includes(NEXT_GROUP_PREFIX))!
                                        const controlGroup: TimelineObject | undefined = nextGroup.children.find(child => child.id.includes(PIECE_CONTROL_INFIX))
								...
								describe('next Part has a "KeepAliveDuration"', () => {
									describe('next Part has a PostRollDuration', () => {
										it('sets TimelineEnable.start to nextGroup.end - Part.keepAliveDuration - next Part.postRoll', () => {
                                            ...
                                            const piece: Piece = EntityDefaultFactory.createPiece({ transitionType: TransitionType.OUT_TRANSITION } as PieceInterface)
                                            const nextPart: Part = EntityDefaultFactory.createPart({ id: 'nextPart', outTransition: { keepAliveDuration }, pieces: [piece] } as PartInterface)

                                            const nextPartSpy: Part = spy(nextPart)
                                            when(nextPartSpy.getTimings()).thenReturn({ postRollDuration } as PartTimings)
                                            ...
                                            expect(controlGroup.enable.start).toBe(`#${nextGroup.id}.end - ${keepAliveDuration} - ${postRollDuration}`)
										})
									})
									...
									describe('next Part does not have a PostRollDuration', () => {
										it('sets TimelineEnable.start to nextGroup.end - next Part.keepAliveDuration', () => {
                                            ...
                                            expect(controlGroup.enable.start).toBe(`#${nextGroup.id}.end - ${keepAliveDuration}`)
										})
									})
								})
							...
							describe('creates TimelineEnable for NO_TRANSITION Piece', () => {
								...
								describe('Piece has a duration', () => {
									it('sets TimelineEnable.duration to Piece.duration', () => {
                                        ...
                                        const piece: Piece = EntityDefaultFactory.createPiece({ duration: 30, transitionType: TransitionType.NO_TRANSITION } as PieceInterface)
                                        const nextPart: Part = EntityDefaultFactory.createPart({ id: 'nextPart', pieces: [piece] } as PartInterface)
								...
								describe('Piece does not have a duration', () => {
									describe('next Part has PostRoll', () => {
										it('sets TimelineEnable.duration previousGroup.end - Part.timings.postRollDuration', () => {
                                            ...
                                            expect(controlGroup.enable.duration).toBe(`#${nextGroup.id} - ${postRollDuration}`)
										})
									})
							...
							describe('controlGroup has TimelineEnable.start === zero && Piece has PreRoll', () => {
								describe('creates PreRollControlGroup for Piece', () => {
									it('sets id to correct id for PreRollControlGroup', () => {
                                        ...
                                        const nextGroup: TimelineObjectGroup = timeline.timelineGroups.find(group => group.id.includes(NEXT_GROUP_PREFIX))!
                                        const controlObject: TimelineObject = nextGroup.children.find(child => child.id.includes(PIECE_CONTROL_INFIX))!
                                        const expectedPreRollIdForPiece: string = `${PIECE_PRE_ROLL_PREFIX}${controlObject.id}`
                                        const preRollObject: TimelineObject | undefined = nextGroup.children.find(child => child.id === expectedPreRollIdForPiece)
									...
									it('sets TimelineEnable.start to "nextGroup.id.star"t', () => {
                                        ...
                                        const nextGroup: TimelineObjectGroup = timeline.timelineGroups.find(group => group.id.includes(NEXT_GROUP_PREFIX))!
                                        const preRollObject: TimelineObject = nextGroup.children.find(child => child.id.includes(`${PIECE_PRE_ROLL_PREFIX}${NEXT_GROUP_PREFIX}`))!
									...
									it('sets an empty layer', () => {
                                        ...
                                        const nextGroup: TimelineObjectGroup = timeline.timelineGroups.find(group => group.id.includes(NEXT_GROUP_PREFIX))!
                                        const preRollObject: TimelineObject = nextGroup.children.find(child => child.id.includes(PIECE_PRE_ROLL_PREFIX))!
									...
									it('updates controlPiece to start at PreRollControlGroup + Piece.preRollDuration', () => {
                                        ...
                                        expect(controlGroup.enable.start).toBe(`#${preRollObject.id} + ${piece.preRollDuration}`)
									})
								})
							})
						})
						...
						describe('create a Piece child group on the next group', () => {
							it('sets correct Piece group id for Piece on next group', () => {
                                ...
                                const childGroup: TimelineObject | undefined = nextGroup.children.find(child => child.id === expectedChildGroupIdForPiece)
							...
							it('sets correct parentGroup id', () => {
                                ...
                                const nextGroup: TimelineObjectGroup = timeline.timelineGroups.find(group => group.id.includes(NEXT_GROUP_PREFIX))!
                                const childGroup: TimelineObject = nextGroup.children.find(child => child.id.includes(PIECE_GROUP_INFIX))!
							...
							describe('Piece has PreRoll', () => {
								it('sets TimelineEnable.start PieceControlGroup.start - Piece.preRollDuration', () => {
                                    ...
                                    const piece: Piece = EntityDefaultFactory.createPiece({ preRollDuration: 15, transitionType: TransitionType.NO_TRANSITION } as PieceInterface)
                                    const nextPart: Part = EntityDefaultFactory.createPart({ id: 'nextPart', pieces: [piece] } as PartInterface)
							...
							describe('Piece has PostRoll', () => {
								it('sets TimelineEnable.end to PieceControlGroup.end - Piece.postRollDuration', () => {
                                    ...
                                    const piece: Piece = EntityDefaultFactory.createPiece({ postRollDuration: 40, transitionType: TransitionType.NO_TRANSITION } as PieceInterface)
                                    const nextPart: Part = EntityDefaultFactory.createPart({ id: 'nextPart', pieces: [piece] } as PartInterface)
							...
							describe('Piece has a TimelineObject', () => {
								it('sets the id of the TimelineObject to be pieceChildGroup.id_piece.id_timelineObject.id', () => {
                                    ...
                                    const nextGroup: TimelineObjectGroup = timeline.timelineGroups.find(group => group.id.includes(NEXT_GROUP_PREFIX))!
                                    const childGroup: TimelineObjectGroup = nextGroup.children.find(child => child.id.includes(PIECE_GROUP_INFIX))! as TimelineObjectGroup
                                    const result: TimelineObject = childGroup.children[0]
					...
					describe('next Part does not have any Pieces', () => {
						it('does not create any groups for the Pieces', () => {
                            ...
                            const nextGroup: TimelineObjectGroup = timeline.timelineGroups.find(group => group.id.includes(NEXT_GROUP_PREFIX))!
                            const childGroup: TimelineObject | undefined = nextGroup.children.find(child => child.id.includes(PIECE_GROUP_INFIX))
					...
					describe('next Part has Pieces', () => {
						describe('next Part has five Pieces, but one of them is an infinite Piece', () => {
							it('only creates groups for four Pieces', () => {
                                ...
                                const nextPart: Part = EntityDefaultFactory.createPart({ id: 'nextPart', pieces } as PartInterface)
                                const segment: Segment = EntityDefaultFactory.createSegment({ parts: [activePart, nextPart]} as SegmentInterface)
                                const rundown: Rundown = EntityDefaultFactory.createActiveRundown([segment])
                                ...
                                const nextGroup: TimelineObjectGroup = timeline.timelineGroups.find(group => group.id.includes(NEXT_GROUP_PREFIX))!
                                const controlGroups: TimelineObject[] = nextGroup.children.filter(child => child.id.includes(PIECE_CONTROL_INFIX))
			...
			describe('active Part does not have an expected duration longer than zero', () => {
				it('does not create a group for next Part', () => {
                    const activePart: Part = EntityDefaultFactory.createPart({ id: 'activePart', autoNext: true } as PartInterface)
                    const nextPart: Part = EntityDefaultFactory.createPart({ id: 'nextPart' } as PartInterface)
                    const segment: Segment = EntityDefaultFactory.createSegment({ parts: [activePart, nextPart]} as SegmentInterface)
                    const rundown: Rundown = EntityDefaultFactory.createActiveRundown([segment])
                    ...
                    const nextGroup: TimelineObjectGroup | undefined = timeline.timelineGroups.find(group => group.id.includes(NEXT_GROUP_PREFIX))
		...
		describe('Rundown has an infinite Piece', () => {
			describe('infinite Piece is an "inTransition" Pieces', () => {
				it('does not create an infinite Piece group', () => {
					const infinitePiece: Piece = EntityDefaultFactory.createPiece({ transitionType: TransitionType.IN_TRANSITION, pieceLifespan: PieceLifespan.STICKY_UNTIL_RUNDOWN_CHANGE } as PieceInterface)
            ...
            describe('infinite Piece is an "outTransition" Pieces', () => {
                it('does not create an infinite Piece group', () => {
                    const infinitePiece: Piece = EntityDefaultFactory.createPiece({ transitionType: TransitionType.OUT_TRANSITION, pieceLifespan: PieceLifespan.STICKY_UNTIL_RUNDOWN_CHANGE } as PieceInterface)
                    const segment: Segment = createSegmentWithPieces([infinitePiece])
                    const rundown: Rundown = EntityDefaultFactory.createActiveRundown([segment])
			...
			describe('infinite Piece is not a "transition" Piece"', () => {
				describe('infinite Piece belongs to the active Part', () => {
					it('does not create infinite groups for Piece', () => {
                        ...
                        const infinitePiece: Piece = EntityDefaultFactory.createPiece({ partId: activePartId, transitionType: TransitionType.NO_TRANSITION, pieceLifespan: PieceLifespan.STICKY_UNTIL_RUNDOWN_CHANGE } as PieceInterface)
                        const activePart: Part = EntityDefaultFactory.createPart({ id: activePartId, pieces: [infinitePiece] } as PartInterface)
                        const segment: Segment = EntityDefaultFactory.createSegment({ parts: [activePart] } as SegmentInterface)
                        const rundown: Rundown = EntityDefaultFactory.createActiveRundown([segment])
				...
				describe('infinite Piece does not belong to the active Part', () => {
					describe('infinite Piece does not have an executedAt larger than zero', () => {
						it('throws an error', () => {
                            ...
                            const infinitePiece: Piece = EntityDefaultFactory.createPiece({ partId: 'randomPartId', transitionType: TransitionType.NO_TRANSITION, pieceLifespan: PieceLifespan.STICKY_UNTIL_RUNDOWN_CHANGE } as PieceInterface)
					...
					describe('infinite Piece has an executedAt larger than zero', () => {
						...
						describe('creates an infinite group for Piece', () => {
							it('sets correct infinite group id', () => {
								...
								const expectedInfinitePieceGroupId: string = `${INFINITE_GROUP_PREFIX}${segment.getParts()[0].id}_${infinitePiece.id}`
                                const infiniteGroup: TimelineObjectGroup | undefined = timeline.timelineGroups.find(group => group.id === expectedInfinitePieceGroupId)
							...
							it('sets priority to medium', () => {
                                ...
                                const infiniteGroup: TimelineObjectGroup = timeline.timelineGroups.find(group => group.id.includes(INFINITE_GROUP_PREFIX))!
							...
							it('sets the layer to Piece.layer', () => {
                                ...
                                const infinitePiece: Piece = EntityDefaultFactory.createPiece({ layer, partId: 'randomPartId', transitionType: TransitionType.NO_TRANSITION, pieceLifespan: PieceLifespan.STICKY_UNTIL_RUNDOWN_CHANGE } as PieceInterface)
                            ...
                            describe('infinite Piece has a TimelineObject', () => {
                                it('sets the id of the TimelineObject to be infinitePieceGroup.id_piece.id_timelineObject.id', () => {
                                    ...
                                    const infinitePiece: Piece = EntityDefaultFactory.createPiece({ timelineObjects: [timelineObject], partId: 'randomPartId', transitionType: TransitionType.NO_TRANSITION, pieceLifespan: PieceLifespan.STICKY_UNTIL_RUNDOWN_CHANGE } as PieceInterface)
									...
									const result: TimelineObject | undefined = infiniteGroup.children.find(child => child.id === expectedTimelineObjectId)
                                ...
                                it('sets the group of the TimelineObject to be the infinite Piece group', () => {
                                    ...
                                    const infiniteGroup: TimelineObjectGroup = timeline.timelineGroups.find(group => group.id.includes(INFINITE_GROUP_PREFIX))!
                                    const expectedTimelineObjectId: string = `${infiniteGroup.id}_${infinitePiece.id}_${timelineObject.id}`
                                    const result: TimelineObject = infiniteGroup.children.find(child => child.id === expectedTimelineObjectId)!
                            ...
                            describe('Piece has five TimelineObjects', () => {
                                it('adds all five TimelineObjects to the children of the infinite Piece group', () => {
                                    ...
                                    const infinitePiece: Piece = EntityDefaultFactory.createPiece({ timelineObjects, partId: 'randomPartId', transitionType: TransitionType.NO_TRANSITION, pieceLifespan: PieceLifespan.STICKY_UNTIL_RUNDOWN_CHANGE } as PieceInterface)
		...
		describe('Rundown has multiple valid infinite Pieces', () => {
			it('creates infinite groups for all infinite Pieces', () => {
                const infinitePieces: Piece[] = [
                    EntityDefaultFactory.createPiece({ id: 'infiniteOne', layer: 'layerOne', partId: 'randomPartId', transitionType: TransitionType.NO_TRANSITION, pieceLifespan: PieceLifespan.STICKY_UNTIL_RUNDOWN_CHANGE } as PieceInterface),
                    EntityDefaultFactory.createPiece({ id: 'infiniteTwo', layer: 'layerTwo', partId: 'randomPartId', transitionType: TransitionType.NO_TRANSITION, pieceLifespan: PieceLifespan.STICKY_UNTIL_RUNDOWN_CHANGE } as PieceInterface),
                    EntityDefaultFactory.createPiece({ id: 'infiniteThree', layer: 'layerThree', partId: 'randomPartId', transitionType: TransitionType.NO_TRANSITION, pieceLifespan: PieceLifespan.STICKY_UNTIL_RUNDOWN_CHANGE } as PieceInterface),
				]
