...
export class SuperflyTimelineBuilder implements TimelineBuilder {
	...
	public buildTimeline(rundown: Rundown): Timeline {
		...
		const activePartTimelineGroup: ActivePartTimelineObjectGroup = this.createTimelineObjectGroupForActivePart(rundown)
		timeline.timelineGroups.push(activePartTimelineGroup)

		const previousPart: Part | undefined = rundown.getPreviousPart()
		if (previousPart) {
			const previousPartTimelineGroup: TimelineObjectGroup = this.createTimelineObjectGroupForPreviousPart(previousPart, rundown.getActivePart(), activePartTimelineGroup)
			timeline.timelineGroups.push(previousPartTimelineGroup)
		}

        const infinitePiecesTimelineGroups: TimelineObjectGroup[] = this.createTimelineObjectGroupsForInfinitePieces(rundown)
        timeline.timelineGroups.push(...infinitePiecesTimelineGroups)

        if (activePartTimelineGroup.shouldAutoNextAtPointInTime > 0) {
            const nextPartTimeline: TimelineObjectGroup = this.createTimelineForNextPart(rundown, activePartTimelineGroup)
			timeline.timelineGroups.push(nextPartTimeline)

			if (Number.isNaN(activePartTimelineGroup.enable.duration)) {
				throw new UnsupportedOperation(`Duration of activePartTimelineGroup must be a number! Got: ${activePartTimelineGroup.enable.duration}`)
			}
			timeline.autoNext = { pointInTimeToTakeNext: activePartTimelineGroup.shouldAutoNextAtPointInTime - rundown.getNextPart().getTimings().previousPartContinueIntoPartDuration }
        }
	...
	private createTimelineObjectGroupForActivePart(rundown: Rundown): ActivePartTimelineObjectGroup {
		...
		if (activePart.autoNext && !!activePart.expectedDuration) {
			currentPartEnable.duration = activePart.expectedDuration + activePart.getTimings().delayStartOfPiecesDuration
		...
		activeGroup.children = activePart.getPieces().flatMap(piece => this.generateGroupsAndTimelineObjectsForPiece(piece, activePart, activeGroup))
		return activeGroup
	}

	private generateGroupsAndTimelineObjectsForPiece(piece: Piece, part: Part, parentGroup: TimelineObjectGroup): TimelineObject[] {
		...
		if (pieceEnable.start === 'now') {
			...
			throw new UnsupportedOperation(`Found an enable.start="start" for control for Piece: ${piece.id}. We are trying to avoid those if possible.`)
		}
		...
		const childGroupForPiece: TimelineObjectGroup = this.createTimelineObjectGroupForPiece(parentGroup, piece, controlForPiece)
		timelineObjectsToReturn.push(childGroupForPiece)

		if (this.shouldPieceHavePreRollGroup(controlForPiece, piece)) {
			const preRollControlForPiece: TimelineObjectControl = this.createPreRollGroupForControl(controlForPiece, parentGroup)
		...
		childGroupForPiece.children = piece.timelineObjects.map(timelineObject => this.mapToTimelineObjectForPieceGroup(timelineObject, childGroupForPiece, piece))

		return timelineObjectsToReturn
	}

	private createTimelineEnableForPiece(part: Part, piece: Piece, parentGroup: TimelineObjectGroup): TimelineEnable | undefined {
		...
		switch (piece.transitionType) {
			...
			case TransitionType.OUT_TRANSITION: {
				...
				const postRollDurationPostFix: string = partCalculatedTimings.postRollDuration ? ` - ${partCalculatedTimings.postRollDuration}` : ''
				return {
			...
			case TransitionType.NO_TRANSITION: {
				...
				// TODO: This hurts...
				const duration: string | number = partCalculatedTimings.postRollDuration && !piece.duration
			...
			default: {
				throw new UnsupportedOperation(`Unknown TransitionType "${piece.transitionType}" for Piece: ${piece.id}`)
			}
		}
	}

	private createTimelineObjectControl(parentGroup: TimelineObjectGroup, piece: Piece, pieceEnable: TimelineEnable): TimelineObjectControl {
		return {
	...
	private createTimelineObjectGroupForPiece(parentGroup: TimelineObjectGroup, piece: Piece, controlForPiece: TimelineObjectControl): TimelineObjectGroup {
		return {
	...
	private createPreRollGroupForControl(controlObject: TimelineObjectControl, parentGroup: TimelineObjectGroup): TimelineObjectControl {
		return {
			id: `${PIECE_PRE_ROLL_PREFIX}${controlObject.id}`,
			enable: {
				// start: parentGroup.enable.start
				// start: Date.now()
	...
	private mapToTimelineObjectForPieceGroup(timelineObject: TimelineObject, childGroupForPiece: TimelineObjectGroup, piece: Piece) {
	...
	private createTimelineObjectGroupForPreviousPart(previousPart: Part, activePart: Part, activeGroup: TimelineObjectGroup): TimelineObjectGroup {
        // TODO: This if-statement shouldn't be necessary in production
        if (previousPart.getExecutedAt() <= 0) {
            throw new Error(`Previous Part: ${previousPart.name} does not have a valid "executedAt" - something went wrong when setting the previous Part.`)
        }
		...
		previousGroup.children = previousPart.getPiecesWithLifespan([PieceLifespan.WITHIN_PART]).flatMap(piece => this.generateGroupsAndTimelineObjectsForPiece(piece, previousPart, previousGroup))
    ...
    private createTimelineObjectGroupsForInfinitePieces(rundown: Rundown): TimelineObjectGroup[] {
		...
		rundown.getInfinitePieces()
            .filter(piece => piece.transitionType === TransitionType.NO_TRANSITION /* TODO: && filter for not disabled - if that becomes a thing */)
			.filter(piece => piece.partId !== activePart.id)
            .forEach(piece => {
                if (piece.getExecutedAt() === 0) {
                    throw new UnsupportedOperation(`Found infinite Piece: ${piece.id} without an "executedAt". Infinite Pieces must have an "executedAt"!`)
                }

				if (piece.pieceLifespan === PieceLifespan.SPANNING_UNTIL_SEGMENT_END || piece.pieceLifespan === PieceLifespan.SPANNING_UNTIL_RUNDOWN_END) {
    ...
    private createTimelineForNextPart(rundown: Rundown, activeGroup: TimelineObjectGroup): TimelineObjectGroup {
        ...
        nextGroup.children = nextPart.getPiecesWithLifespan([PieceLifespan.WITHIN_PART]).flatMap(piece => this.generateGroupsAndTimelineObjectsForPiece(piece, nextPart, nextGroup))
